...
Requestor::Requestor(const APISession& session, DateManager* _m_dateManager)
: ActiveTickServerRequestor(session.GetSessionHandle())
{
	m_dateManager = _m_dateManager;
}
...
/*virtual*/ void Requestor::OnATTickHistoryDbResponse(uint64_t origRequest, ATTickHistoryResponseType responseType, LPATTICKHISTORY_RESPONSE pResponse)
{
	...
	// ofstream responseLimit;
	// fileTrades.open(("/home/william/projects/ActiveTickHttpInterface/ActiveTickHttpInterface/data/20160114/responseLimit.csv").c_str(), ios_base::app | ios_base::ate | ios_base::out);

	fileTrades.open(("/home/william/projects/stocks/data/" + m_dateManager->currentDate() + "/" + symbol + ".csv").c_str(), ios_base::app | ios_base::ate | ios_base::out);
	...
	if(parser.MoveToFirstRecord())
	{
		while(true)
		{
			...
			case TickHistoryRecordTrade:
				...
				bufferLen = sprintf(buffer, "%0.2d%0.2d%0.2d%0.3d,%0.*f,%u,%u,%d\r\n",
	...
	if(pResponse->nextOffset != 0xffffffffffffffffULL && pResponse->nextOffset != 0xfffffffffffffff0ULL && pResponse->nextOffset != 0) {
		printf("next");
		SendATTickHistoryDbRequest(pResponse->symbol, true, false, 10, pResponse->nextOffset, pResponse->offsetDatabaseDate, DEFAULT_REQUEST_TIMEOUT);
	}
}
