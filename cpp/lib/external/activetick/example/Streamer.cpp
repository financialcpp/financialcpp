...
Streamer::Streamer(const APISession& session, DateManager* _m_dateManager)
: ActiveTickStreamListener(session.GetSessionHandle())
, Requestor(session, _m_dateManager)
, m_session(session)
{
}
...
/*virtual*/ void Streamer::OnATStreamTradeUpdate(LPATQUOTESTREAM_TRADE_UPDATE pUpdate)
{
	...
	// 	pUpdate->lastSize);
	// printf("\n");
	// data.SYMB["AAPL"].volume += pUpdate->lastSize;
	// printf("AAPL: %d", data.SYMB["AAPL"].volume);
	// printf("\n");
	...
	/*printf("RECV: [%0.2d:%0.2d:%0.2d] STREAMTRADE(%u) [symbol:%s last:%0.*f lastSize:%d]\n",
		...
		pUpdate->lastPrice.precision, pUpdate->lastPrice.price,
		pUpdate->lastSize);*/
		// std::string s = Helper::ConvertString(pUpdate->symbol.symbol, _countof(pUpdate->symbol.symbol));
		// // data.SYMB[s].price += pUpdate->lastPrice;
		// data.SYMB[s].volume += pUpdate->lastSize;
		// data.SYMB[s].count = data.SYMB[s].count + 1;
		// std::cout << s << std::endl;
		// std::cout << "Price: " << data.SYMB[s].price << std::endl;
		// std::cout << "Volume: " << data.SYMB[s].volume << std::endl;
		// std::cout << "Count: " << data.SYMB[s].count << std::endl;
}
